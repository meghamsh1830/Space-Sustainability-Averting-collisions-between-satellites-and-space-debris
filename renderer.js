...
], function (Camera, declare, externalRenderers) {
  // Enforce strict mode
  "use strict";
  ...
  return declare([], {
    ...
    setup: function (context) {
      ...
      this.renderer = new THREE.WebGLRenderer({
        context: context.gl,
        premultipliedAlpha: TextTrackCue,
      });
      ...
      worker.onmessage = function (e) {
        ...
        for (var i = 0; i < this.satellites.length; i++) {
          ...
          vertex.push(position.getX(i), position.getY(i), position.getZ(i));
        }
        this.selected.geometry.removeAttribute("position");
        this.selected.geometry.addAttribute(
          "position",
          new THREE.Float32Attribute(vertex, 3)
        );
        ...
        if (this.satelliteHover !== null) {
          ...
          this.hover.geometry.addAttribute(
            "position",
            new THREE.Float32Attribute(vertex2, 3)
          );
        ...
        if (this.satelliteIdentified !== null) {
          ...
          this.identified.geometry.addAttribute(
            "position",
            new THREE.Float32Attribute(vertex3, 3)
          );
    ...
    mousemove: function (x, y) {
      ...
      if (satellite === null && this.satelliteHover === null) {
        return;
      }
      if (satellite === this.satelliteHover) {
        return;
      }
    ...
    showOrbit: function () {
      ...
      for (var i = 0; i <= TRAJECTORY_SEGMENTS; i++) {
        ...
        var eci = this.getSatelliteLocation(
          this.satelliteIdentified.satrec,
          date
        );
        if (
          eci === null ||
      ...
      this.identified.geometry.addAttribute(
        "position",
        new THREE.Float32Attribute(vertex.slice(0, 3), 3)
      );
